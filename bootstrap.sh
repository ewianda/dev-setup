...
install_vim() {
	echo "y" | sudo apt-get remove --purge \
		vim \
		vim-runtime \
		gvim \
		vim-tiny \
...
main() {
	echo "Installing"
	(sudo apt update -yqq && sudo apt install sudo curl -yqq) || true
	sudo apt install default-jre shellcheck build-essential libncurses-dev tmux python-dev python3-dev -yqq
	install_git
	install_node
	install_fzf
	install_vim
